...
public class SimpleCalendar extends JFrame { // Definicija klase koja nasljeđuje JFrame
    ...
    public String getKorisnickoIme() {//konstruktor za dobiti korisničko ime
    ...
    public SimpleCalendar(String korisnickoIme) { // Konstruktor glavne klase
        ...
        setTitle("Simple Calendar"); // Postavlja naslov prozora
            ...
            clientField.addActionListener(new ActionListener() {
                ...
                public void actionPerformed(ActionEvent e) {
                    int response = JOptionPane.showConfirmDialog(
                            ...
                            "Jeste li sigurni da želite rezervirati ovaj termin?", // Poruka dijaloga
                    ...
                    if (response == JOptionPane.YES_OPTION) {
                        // Ako je odgovor "Da", postavite korisničko ime i onemogućite gumb
                        clientField.setText(korisnickoIme);
                        clientField.setEnabled(false); // Onemogućite gumb
                        statusField.setBackground(Color.RED); // Promijenite boju statusa
                        saveAppointments(day, timePanel, clientField, timeSlot);
                    } else {
                        // Ako je odgovor "Ne", ništa se ne događa
    ...
    private void loadAppointments(int day, JPanel timePanel) {
        try (Connection conn = DBConnection.getConnection()) { // Otvara vezu s bazom
            ...
            JOptionPane.showMessageDialog(this, "Došlo je do greške prilikom učitavanja podataka: " + ex.getMessage()); // Obavještava korisnika o grešci
        }
    }



    private void saveAppointments(int day, JPanel timePanel, JButton clickedButton, String Timeslot) { // Metoda za spremanje termina
        try (Connection conn = DBConnection.getConnection()) { // Otvara vezu s bazom
            ...
            // Provjeravamo da li je gumb za rezervaciju kliknut
            if (!clickedButton.getText().equals("Rezerviraj termin")) {
                String client = clickedButton.getText(); // Uzmi ime klijenta s gumba
            ...
            JOptionPane.showMessageDialog(this, "Podaci su spremljeni!"); // Obavještava korisnika da je sve uspješno spremljeno
        } catch (SQLException ex) {
            ex.printStackTrace(); // Ispisuje greške u konzolu
            JOptionPane.showMessageDialog(this, "Došlo je do greške prilikom spremanja podataka: " + ex.getMessage()); // Obavještava korisnika o grešci
        }
    }
